...
#Here basically we will the csvs and quuee them . 
...
def read_audio_csv(filename_queue):
  ...
  WIDE = FLAGS.WIDE
  CONV_KEEP_PROB = FLAGS.CONV_KEEP_PROB
  reader = tf.TextLineReader()
  key, value = reader.read(filename_queue)
  defaultVal = [[0.] for idx in range(WIDE*FEATURE_DIM + OUT_DIM)]

  fileData = tf.decode_csv(value, record_defaults=defaultVal)    #here WIDE=fileData[-1]   so we can easily maintain the dynamic behavior here we have to filter all zeros 
  features = fileData[:WIDE*FEATURE_DIM]           #here 
  features = tf.reshape(features, [WIDE, FEATURE_DIM])
  labels = fileData[WIDE*FEATURE_DIM:]
  return features, labels        #Here we also can pass an array where it say the length of the inputs variable lengths removing the zeros 
#Return shpuld have one more thing length of the seqeunce . 
